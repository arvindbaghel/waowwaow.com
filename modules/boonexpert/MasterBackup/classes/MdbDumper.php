...
if (!file_exists(PATH) && !$is_safe_mode) {
    mkdir(PATH, 0777) || trigger_error("Unable to make backup catalogue", E_USER_ERROR);
}
...
class dumper {
	function dumper() {
		...
		// MySQL version, for example 40101
	...
	function backup() {
		...
		$buttons = "<A ID=save HREF='' STYLE='display: none;'>Download file</A> &nbsp; ";
$buttons .= "<INPUT ID=back TYPE=button VALUE='Back' DISABLED onClick=\"history.back();\">";
		echo tpl_page(tpl_process("Making DB backup"), $buttons);
		...
		if (!$db) {
			echo tpl_l("ERROR! Database is not specified!", C_ERROR);
			echo tpl_enableBack();
		    exit;
		}
		echo tpl_l("Connection attemp to DB `{$db}`.");
		...
		echo tpl_l("Create a file with a backup database:<BR>\\n  -  {$this->filename}");
		...
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysql_query("SET NAMES '" . CHARSET . "'") or trigger_error ("Can not change the character set.<BR>" . mysql_error(), E_USER_ERROR);
        ...
        foreach ($tables AS $table){
			// Set the character set encoding of the corresponding table
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					mysql_query("SET NAMES '" . $tab_charset[$table] . "'") or trigger_error ("Unable to change connection character set.<BR>" . mysql_error(), E_USER_ERROR);
					echo tpl_l("Character set `" . $tab_charset[$table] . "`.", C_WARNING);
					$last_charset = $tab_charset[$table];
				}
				else{
					echo tpl_l('Encoding of the connection and table does not match:', C_ERROR);
        ...
        echo tpl_l(str_repeat("-", 60));
        $this->fn_close($fp);
		echo tpl_l("DB backup `{$db}` has been created.", C_RESULT);
		echo tpl_l("DB size:       " . $this->dfilesize($this->size), C_RESULT);
		$filesize = $this->dfilesize(filesize(PATH . $this->filename));
		echo tpl_l("File size: {$filesize}", C_RESULT);
		echo tpl_l("Tables processed: {$tabs}", C_RESULT);
		echo tpl_l("Strings processed:   " . fn_int($tabinfo[0]), C_RESULT);
		echo "<SCRIPT> document.getElementById('back').disabled = 0;
window.parent.document.getElementById('filelist_frame').contentDocument.location='MdbFiles.php';
</SCRIPT>";


	}

function dfilesize($size) {
	...
	function restore(){
		if (!isset($_POST)) {$this->main();}
		set_error_handler("SXD_errorHandler");
		$buttons = "<INPUT ID=back TYPE=button VALUE='Back' DISABLED onClick=\"history.back();\" style='cursor:pointer'>";
		echo tpl_page(tpl_process("Restore database from backup"), $buttons);
		...
		if(preg_match("/^(.+?)\.sql(\.(bz2|gz))?$/", $file, $matches)) {
			...
			if (!file_exists(PATH . "/{$file}")) {
    		    echo tpl_l("ERROR! File is not found", C_ERROR);
				echo tpl_enableBack();
    		    exit;
    		}
			...
			echo tpl_l("Error! File is not specified", C_ERROR);
			echo tpl_enableBack();
		    exit;
		}
		echo tpl_l(str_repeat("-", 60));
		...
		// Installing the connection charset
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // The default encoding, if the dump does not specify the encoding
			mysql_query("SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Unable to change the character set.<BR>" . mysql_error(), E_USER_ERROR);
			echo tpl_l("Character set is `" . $this->restore_charset . "`.", C_WARNING);
		...
		while(($str = $this->fn_read_str($fp)) !== false){
			...
			if ($sql) {
			    if (preg_match("/;$/", $str)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										mysql_query("SET NAMES '" . $charset[2] . "'") or trigger_error ("Unable to change the character set.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
										$cache .= tpl_l("Character set is `" . $charset[2] . "`.", C_WARNING);
										$last_charset = $charset[2];
									}
									else{
										$cache .= tpl_l('Encoding of the connection and table does not match:', C_ERROR);
										$cache .= tpl_l('Table `'. $table .'` -> ' . $charset[2] . ' (connection '  . $this->restore_charset . ')', C_ERROR);
									}
								}
								// Change the encoding unless specified to force the encoding
							...
							elseif(CHARSET == 'auto'){ // Insert encoding for the tables if it is not specified and installed auto coding
								$sql .= ' DEFAULT CHARSET=' . $this->restore_charset . $this->restore_collate;
								if ($this->restore_charset != $last_charset) {
									mysql_query("SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Unable to change the character set.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
									$cache .= tpl_l("Character set is `" . $this->restore_charset . "`.", C_WARNING);
									$last_charset = $this->restore_charset;
								}
							}
						}
				    ...
				    // Set the encoding in case there is no CREATE TABLE
					elseif($this->mysql_version > 40101 && empty($last_charset)) { 
						mysql_query("SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Unable to change the character set.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
						echo tpl_l("Character set is `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
            		$insert = '';
    			...
    			if ($execute) {
            		$q++;
            		mysql_query($sql) or trigger_error ("Invalid request.<BR>" . mysql_error(), E_USER_ERROR);
		...
		if (isset($info[3])) echo tpl_l("Creation date: {$info[3]}", C_RESULT);
		...
		$this->size = $this->dfilesize(filesize(PATH . $this->filename));
		$this->comp = $this->SET['comp_method'] * 10 + $this->SET['comp_level'];
		echo "<SCRIPT> document.getElementById('back').disabled = 0;
		window.parent.document.getElementById('filelist_frame').contentDocument.location.reload(true);
		</SCRIPT>";
	...
	function main(){
		$this->comp_levels = array('1' => '1 (min)', '2' => '2', '3' => '3', '4' => '4','5' => '5 (average)', '6' => '6', '7' => '7', '8' => '8','9' => '9 (max)' /*,'0' => 'No compression'*/);
...
// 		}
		...
		$buttons = "<INPUT TYPE=submit VALUE=Apply>";
...
<TR>
<FORM NAME=skb METHOD=POST ACTION=MdbDumper.php>
<TD VALIGN=TOP STYLE="padding: 8px 8px;">
{$content}
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD STYLE='color: #000' ID=timer></TD>
...
<INPUT TYPE=radio NAME=action VALUE=backup style="cursor:pointer">
BACKUP Dolphin database </LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
...
<LEGEND style="cursor:pointer">
<INPUT TYPE=radio NAME=action VALUE=restore style="cursor:pointer">
RESTORE Dolphin Database </LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>Database:</TD>
...
<TR><TD style='WIDTH:100px'>Table status:</TD><TD WIDTH=300><TABLE WIDTH=100% BORDER=1 CELLPADDING=0 CELLSPACING=0>
<TR><TD BGCOLOR=#FFFFFF><TABLE WIDTH=1 BORDER=0 CELLPADDING=0 CELLSPACING=0 BGCOLOR=#00f6ff ID=st_tab
STYLE="border-right: 1px solid #AAAAAA"><TR><TD HEIGHT=12></TD></TR></TABLE></TD></TR></TABLE></TD></TR>
<TR><TD>General status:</TD><TD><TABLE WIDTH=100% BORDER=1 CELLSPACING=0 CELLPADDING=0>
<TR><TD BGCOLOR=#FFFFFF><TABLE WIDTH=1 BORDER=0 CELLPADDING=0 CELLSPACING=0 BGCOLOR=#00AA00 ID=so_tab
STYLE="border-right: 1px solid #AAAAAA"><TR><TD HEIGHT=12></TD></TR></TABLE></TD>
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>Master Database Backup requires:<BR> - Internet Explorer 5.5+, Mozilla or Opera 8+ (<SPAN ID=sie>-</SPAN>)<BR> - including the implementation of JavaScript  (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
...
return <<<HTML
<FIELDSET>
<LEGEND>Error connecting to database</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
